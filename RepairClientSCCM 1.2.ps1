...
{
    if((Get-Process -Name "ccmsetup" -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue)) {Stop-Process -Name "ccmsetup" -Force -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue}
    Write-AdvancedLogFile -Message 'Processo ccmsetup encerrado' -Component 'stopAllProcesses' -LogFile $LogFile
    
    if((Get-Process -Name "ccmexec" -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue)) {Stop-Process -Name "CcmExec" -Force -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue}
    Write-AdvancedLogFile -Message 'Processo ccmexec encerrado' -Component 'stopAllProcesses' -LogFile $LogFile
    
    if((Get-Process -Name "rundll32" -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue)) {Stop-Process -Name "rundll32" -Force -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue}
    Write-AdvancedLogFile -Message 'Processo rundll32 encerrado' -Component 'stopAllProcesses' -LogFile $LogFile
    
    if((Get-Process -Name "msiexec" -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue)) {Stop-Process -Name "msiexec" -Force -ErrorAction SilentlyContinue -WarningAction SilentlyContinue -InformationAction SilentlyContinue}
    Write-AdvancedLogFile -Message 'Processo msiexec encerrado' -Component 'stopAllProcesses' -LogFile $LogFile
    
    Start-Sleep -Seconds 3 -ErrorAction SilentlyContinue -WarningAction SilentlyContinue  
    return $true
}
...
{
    $unreg = cmd.exe /c "C:\Windows\system32\msiexec.exe /unregister"
    $reg = cmd.exe /c "C:\Windows\system32\msiexec.exe /regserver"
    return $true
}
...
function repairWMI
{
    ...
    $winmgmtStart = cmd.exe /c "SC config winmgmt start= disabled"
    ...
    $windir = cmd.exe /c "%windir%\system32\wbem\winmgmt /resyncperf"
    ...
    $regserver = cmd.exe /c "%windir%\system32\wbem\wmiprvse /regserver"

    $winmgmtAuto = cmd.exe /c "sc config winmgmt start= auto"
    ...
    while($finished -eq $false)
    {
        ...
        Finally
        {
            if($erro -eq $false)
            {
                ...
                cleanRegistryKey -path "HKLM:\SOFTWARE\Microsoft\SystemCertificates\SMS\Certificates"
                if (Test-Path 'HKLM:\SOFTWARE\Microsoft\SystemCertificates\SMS\Certificates') {Write-AdvancedLogFile -Message 'Erro na limpeza do registro: HKLM:\SOFTWARE\Microsoft\SystemCertificates\SMS\Certificates' -Component 'UninstallClient' -LogFile $LogFile}
                else {Write-AdvancedLogFile -Message 'Limpeza do registro realizada com sucesso: HKLM:\SOFTWARE\Microsoft\SystemCertificates\SMS\Certificates' -Component 'UninstallClient' -LogFile $LogFile}
                ...
                $wmiRemoveCCM = cmd.exe /c "wmic /Namespace:\\root path __Namespace where Name="ccm" delete"
